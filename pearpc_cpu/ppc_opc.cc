...
{
	gCPU.msr = newmsr;
	fprintf(stderr, (char *)"Warning: msr set, but doesn't do anything");
}
...
void ppc_opc_eciwx()
{
	fprintf(stderr, "eciwx unimplemented.\n");
}
/*
...
void ppc_opc_ecowx()
{
	fprintf(stderr, "ecowx unimplemented.\n");
}
#endif
/*
...
void ppc_opc_mcrfs()
{
	uint32 crfD, crfS, ign;
	PPC_OPC_TEMPL_X(gCPU.current_opc, crfD, crfS, ign);
	crfD = (crfD >> 2);
	crfS = (crfS >> 2);
	gCPU.cr &= (~(0xF0000000 >> (crfD * 4)));
	ign = ((gCPU.fpscr & ((0xF << 28) >> (crfS * 4))));
	crfD > crfS ? ign = (ign << (crfD - crfS)) : ign = (ign >> (crfS - crfD));
	gCPU.cr |= ign;
	gCPU.fpscr &= (~(0xF0000000 >> (crfS * 4)));
}
/*
...
void ppc_opc_mcrxr()
{
	...
	gCPU.cr &= (~(0xF0000000 >> (crfD * 4)));
	gCPU.cr |= (((gCPU.xer & 0xF) << 28) >> (crfD * 4));
	gCPU.xer &= ~0xF;
}
/*
...
void ppc_opc_mffsx()
{
	...
	PPC_OPC_ASSERT(rA==0 && rB==0);
	gCPU.fpr[frD] |= (gCPU.fpscr & 0xFFFFFFFF);
	gCPU.fpr[frD] &= 0xFFFFFFFF;
	#ifdef G1
	gCPU.fpr[frD] |= (0xFFF80000 << 32);
	#else
	gCPU.fpr[frD] |= ((uint64)0xFFFFFFFF << 32);
	#endif
	if (gCPU.current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		/* clear because there are no errors */
		gCPU.cr &= 0xF0FFFFFF;		
	}
}
/*
...
void ppc_opc_mfspr()
{
	...
	case 31:
		...
		case 30:
			...
			return;
		case 31:
			PPC_OPC_WARN("read from spr %d:%d (unknown) not supported!\n", spr1, spr2);
			gCPU.gpr[rD] = 0;
			return;
		}
	}
...
void ppc_opc_sc()
{
	...
	/* don't emulate graphics card */
	#error "fixme: implement system calls"
