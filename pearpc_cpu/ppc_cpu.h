...
struct PPC_CPU_State {	
	...
	void * bp;
	NATIVEINT stack_size;
	NATIVEINT code_offset;
	void * realcode;
	NATIVEINT code_size;
	#ifdef G5
	uint64 pgpr[32];
...
#endif
#ifdef G5
#define GET for (int i = 0; i < 32; i ++) { \
		gpr[i] = (uint32)(pgpr[i] & 0xFFFFFFFF); \
	}
#define COMMIT for (int i = 0; i < 32; i ++) { \
		pgpr[i] = ((pgpr[i] & 0xFFFFFFFF00000000) | (uint64)gpr[i]); \
	}
#else
#define GET
#define COMMIT
