...
static void ppc_opc_invalid()
{
	if (gCPU.current_opc == 0x00333301) {
		// memset(r3, r4, r5)
		void * dest;
		ppc_effective_to_physical_c(gCPU.gpr[3], PPC_MMU_WRITE, &dest);
		memset_f(dest, (int)gCPU.gpr[4], (size_t)gCPU.gpr[5]);
		gCPU.pc = gCPU.npc;
		return;
	}
	if (gCPU.current_opc == 0x00333302) {
		// memcpy
		size_t size = (size_t)gCPU.gpr[5];
		void *d;
		void *s;
		ppc_effective_to_physical_c(gCPU.gpr[3], PPC_MMU_WRITE, &d);
		ppc_effective_to_physical_c(gCPU.gpr[4], PPC_MMU_READ, &s);
		memcpy(d, s, size);
		gCPU.pc = gCPU.npc;
		return;
	}
	fprintf(stderr, " Bad opcode: %08x (%u:%u)\n",
...
static ppc_opc_function ppc_opc_table_main[64] = {
	...
	&ppc_opc_subfic,	//  8
	#ifdef G1
	&ppc_opc_dozi, /* 9 is dozi on G1(moterola 601) */
	#else
	&ppc_opc_invalid, /* but unvaild on others(as it's POWER only) */
	#endif
	&ppc_opc_cmpli,		// 10
	&ppc_opc_cmpi,		// 11
	&ppc_opc_addic,		// 12
	...
	&ppc_opc_rlwinmx,	// 21
	#ifdef G1
	&power_opc_rlmi,	/* rlmi on G1(moteroler 601 and POWER) */
	#else
	&ppc_opc_invalid,	/* but unvalled elsewere */
	#endif
	&ppc_opc_rlwnmx,	// 23
	&ppc_opc_ori,		// 24
	&ppc_opc_oris,		// 25
