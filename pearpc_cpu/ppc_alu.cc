...
void ppc_opc_addx()
{
	...
	if (rD == 1) {
		if (gCPU.gpr[rA] + gCPU.gpr[rB] > EBP) {
			fprintf(stderr, (char *)"stack pointer(r1) > top of stack");
		}
		else if ((EBP - (gCPU.gpr[rA] + gCPU.gpr[rB])) > (gCPU.stack_size + 210)) {
			realloc(gCPU.bp, ((EBP - (gCPU.gpr[rA] + gCPU.gpr[rB])) + 210));
			gCPU.stack_size = ((EBP - (gCPU.gpr[rA] + gCPU.gpr[rB])) + 210);
		}
	}
...
void ppc_opc_divwx()
{
	...
	PPC_OPC_TEMPL_XO(gCPU.current_opc, rD, rA, rB);
	if (!gCPU.gpr[rB]) {
		fprintf(stderr, "Warning: division by zero @%08x\n", gCPU.pc);
		/*SINGLESTEP("");*/
	}
	sint32 a = gCPU.gpr[rA];
	sint32 b = gCPU.gpr[rB];
